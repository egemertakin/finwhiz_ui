...
def determine_doctype(url: str) -> str:
    ...
    if any(token in url_lower for token in ("publication", "pub")):
        return "publication"
    if "forms-pubs/about-" in url_lower:
        return "form-guide"
    if "retirement-plans" in url_lower:
        return "guide"
    if "taxtopics" in url_lower:
        return "tax-topic"
    if "newsroom" in url_lower:
        return "news"
    if "credits-deductions" in url_lower:
        return "guide"
    if "small-businesses-self-employed" in url_lower:
        return "guide"
...
def ingest_from_config(
    ...
    *, 
    ...
    authority: str = config.get("authority", "irs.gov")
    ...
    LOGGER.info("Processing %d URLs from IRS.gov", len(seeds))
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Normalize IRS documents for FinWhiz ingest"
    )
    parser.add_argument(
        "--config", 
        type=Path, 
        default=Path(__file__).parent / "irs_config.yaml",
        help="Path to irs_config.yaml"
    )
    parser.add_argument(
        "--output-dir", 
        type=Path, 
        default=Path("data/irs"), 
