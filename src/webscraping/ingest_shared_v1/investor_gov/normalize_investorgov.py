"""Driver for Investor.gov document ingestion and normalization."""
...
def determine_doctype(url: str) -> str:
    ...
    if "glossary" in url_lower:
        return "glossary"
    if "fraud" in url_lower or "protect" in url_lower:
        return "fraud-alert"
    if "news-alerts" in url_lower or "bulletins" in url_lower:
        return "bulletin"
    if "introduction-investing" in url_lower:
        return "guide"
    if "financial-tools" in url_lower:
        return "tool"
...
def ingest_from_config(
    ...
    *, 
    ...
    authority: str = config.get("authority", "investor.gov")
    ...
    LOGGER.info("Processing %d URLs from Investor.gov", len(seeds))
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Normalize Investor.gov documents for FinWhiz ingest"
    )
    parser.add_argument(
        "--config", 
        type=Path, 
        default=Path(__file__).parent / "investorgov_config.yaml",
        help="Path to investorgov_config.yaml"
    )
    parser.add_argument(
        "--output-dir", 
        type=Path, 
        default=Path("data/investorgov"), 
